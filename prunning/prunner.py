...
class ModelPrunner:
    ...
    def _make_new_conv(self, conv, filter_index, channel_type="out"):
        if not isinstance(conv, nn.Conv2d):
            raise TypeError("The module is not Conv2d, but {}.".format(type(conv)))
        ...
        if channel_type == "out":
            ...
            raise ValueError("{} should be either 'in' or 'out'.".format(channel_type))
        return new_conv

    def remove_conv_filter(self, path, filter_index):
        conv = self.book.get_module(path)
        logging.info('Prune Conv: {}, Filter: {}, Layer: {}'.format("/".join(path), filter_index, conv))
    ...
    def remove_linear_feature(self, path, feature_index):
        ...
        logging.info('Prune Linear: {}, Filter: {}, Layer: {}'.format("/".join(path), feature_index, linear))
